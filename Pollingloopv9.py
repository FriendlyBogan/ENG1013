...
def display_main_menu(pin, distanceCm,polledData,timeForAverageVel, board):
    """
    ...
    # Process choice
    if choice == 1:
        polledData, timeForAverageVel = normal_mode(distanceCm,polledData,timeForAverageVel, board)
        return pin, distanceCm, polledData, timeForAverageVel
    elif choice == 2:
        newPin, newDistance = authorize_user(pin, distanceCm)
        if newPin is not None:
            return newPin, newDistance,polledData, timeForAverageVel
        else:
            return pin, distanceCm, polledData, timeForAverageVel
    elif choice == 3:
        display_data_observation_menu(polledData, timeForAverageVel)
        return pin, distanceCm, polledData, timeForAverageVel
    elif choice == 0:
...
1
def normal_mode(distanceCm,data_list,timeForAverageVel, board):
    """
    ...
    """
    totalTime = 0
    polledData = []  # Initialize polledData here (check notes)
    timeForAverageVel = 0
    ...
    while True:
        try:
            ...
            timeForAverageVel += pollingTime
            #run the output traffic operation sequence 
            result = traffic_operation_sequence(totalTime, polledData)
            ...
            return polledData, timeForAverageVel
...
def display_data_observation_menu(polledData, timeForAverageVel):
    """
    ...
    print("3: Display the average velocity of the vehicles for the last normal operation.")
    print("0. Return to main menu.\n")

    print(polledData, timeForAverageVel)
    ...
    if choice == 1:
        ...
        if polledData == []:
            ...
            print(f"The average velocity is: {average_velocity(polledData, timeForAverageVel)}")  
    elif choice == 0:
        return
...
def display_distance_to_nearest_vehicle(distance):
    ...
    Function to display the distance to the nearest vehicle in cm.
        ...
        Returns:
            Function has no returns
    """
    print(f"The distance to the nearest vehicle is : {distance:.2f} cm") 


def traffic_operation_sequence(totalTime, sensorDistances):
    """
    ...
    print('lalala', sensorDistances[-1:-4:-1])
    
    if round(totalTime) % 3 == 0:
        display_distance_to_nearest_vehicle(min(sensorDistances[-1:-4:-1]))
...
def main():
    ...
    timeForAverageVel = 0 #is this needed? RESETS FOR EVERY NORMAL OPERATRION RUN
    polledData = []  #is this needed? RESETS FOR EVERY NORMAL OPERATRION RUN
    board = pymata4.Pymata4()

    while True:
        pin, distanceCm, polledData, timeForAverageVel= display_main_menu(pin, distanceCm,polledData, timeForAverageVel, board)
