...
# from FinalVersion import lights
# from FinalVersion import polling_loop
from pymata4 import pymata4
import time

velList = []

# TODO: Add specificRange as a parameter
def ultrasonic_sensor(triggerPin,echoPin,board,ultrasonicData, distanceTimer):
    ...
    # TODO: MAKE IT SO THIS CODE BELOW ONLY OCCURS WHEN STAGE ONE IS RUNNING
    if len(ultrasonicData) > 2:
        velList.append(vel)
        if len(velList) >= 2:
    ...
    # TODO: Starting the code for yellow light extension for specificRange parameter in stage two of normal mode
    # if stage == 'stageTwo':
    #     if ultrasonicData[-1] <= specificRange:
...
def main():
    ...
    ultrasonicData = []
    while True:
        ultrasonicData, distanceTimer = ultrasonic_sensor(triggerPin=9, echoPin=10, board=board, ultrasonicData=ultrasonicData, distanceTimer=distanceTimer)
